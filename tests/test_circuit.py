...
def test_target_conditioned_circuit_token_denoiser_output_shape():
    model = TargetConditionedCircuitTokenDenoiser(T=8, hidden=16, target_dim=10, num_layers=1, num_heads=4)
    q_stack = torch.randn(5, 6, 4)
    q_stack = q_stack / q_stack.norm(dim=-1, keepdim=True)
    t_idx = torch.tensor([1, 2, 3, 4, 5], dtype=torch.long)
    features = torch.randn(5, 10)
